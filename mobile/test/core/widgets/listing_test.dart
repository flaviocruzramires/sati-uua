...
void main() {
  ...
  group('ErrorState', () {
    ...
    testWidgets('sem onRetry não mostra botão', (tester) async {
      await tester.pumpWidget(_wrap(const ErrorState(mensagem: 'Erro')));
  ...
  group('PaginationBar', () {
    testWidgets('mostra label de paginação', (tester) async {
      await tester.pumpWidget(
        _wrap(
          PaginationBar(page: 1, pageSize: 8, total: 20, onPageChanged: (_) {}),
        ),
      );
    ...
    testWidgets('total 0 não renderiza nada', (tester) async {
      await tester.pumpWidget(
        _wrap(
          PaginationBar(page: 1, pageSize: 8, total: 0, onPageChanged: (_) {}),
        ),
      );
  ...
  group('FilterBar', () {
    testWidgets('renderiza filtros filhos', (tester) async {
      await tester.pumpWidget(
        _wrap(
          FilterBar(filters: [const Text('Filtro A'), const Text('Filtro B')]),
        ),
      );
  ...
  group('AppCardListItem', () {
    ...
    testWidgets('chama onTap ao tocar', (tester) async {
      var tapped = false;
      await tester.pumpWidget(
        _wrap(AppCardListItem(titulo: 'Item', onTap: () => tapped = true)),
      );
