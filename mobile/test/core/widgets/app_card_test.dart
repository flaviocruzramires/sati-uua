...
void main() {
  testWidgets('AppCard renderiza filho', (tester) async {
    await tester.pumpWidget(_wrap(
        const AppCard(child: Text('Conteúdo'))));
    expect(find.text('Conteúdo'), findsOneWidget);
  });

  testWidgets('AppCard elevation none sem sombra', (tester) async {
    await tester.pumpWidget(_wrap(
        const AppCard(elevation: AppCardElevation.none, child: SizedBox())));
    final container = tester.widget<Container>(find.byType(Container).first);
    final decoration = container.decoration as BoxDecoration;
    expect(decoration.boxShadow, isEmpty);
  });

  testWidgets('SectionDivider renderiza Divider com espessura 2', (tester) async {
